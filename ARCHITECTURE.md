...
| B4 | **Roll the system clock forward.** | Originally: unlock compared `Time/Until` to `DateTime.Now`, so setting the clock past the end time made the next 10 s tick `stopMe()` and lift the block. **MITIGATION CODE-COMPLETE + UNIT-TESTED (2026-06-13, `a32a0cd`; NOT yet live-verified):** every expiry/self-heal decision now uses a MAC-covered monotonic `[Time] HighWater` high-water mark (advanced at most one tick, never on a jump > 120 s) as `asOf` instead of `DateTime.Now`, so a clock rolled forward past `Until` no longer reads "expired". Pure gates `ClassifyTimeAdvance`/`NextHighWater` parity-pinned; stacks on the B7 fail-closed MAC. | Critical → *(pending live smoke test, then expected Low/Medium)* |
