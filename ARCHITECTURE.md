...
| B4 | **Roll the system clock forward.** | Originally: unlock compared `Time/Until` to `DateTime.Now`, so setting the clock past the end time made the next 10 s tick `stopMe()` and lift the block. **PARTIAL MITIGATION (2026-06-13, `a32a0cd`):** every expiry/self-heal decision uses a MAC-covered `[Time] HighWater` mark as `asOf` instead of `DateTime.Now`; a single clock jump > 120 s past `Until` is refused (`ClassifyTimeAdvance` → `ForwardJump` → mark not advanced), and tampering the mark fails closed post-B7. **OPEN — P1 within-ceiling clock-creep (audit 2026-06-13, verifier-confirmed):** the advance is capped per *step* (≤ 120 s) but NOT bound to real elapsed time — there is no monotonic clock. An attacker who nudges the clock +119 s right before each 10 s real tick walks `HighWater` toward `Until` ~12× faster than honest time and lifts a block early (a 1 h block in ~5 min; restart adds +120 s/`OnStart`). **Fix (not yet implemented — timing-sensitive, needs the live B4 clock test to tune):** bound each tick's credited advance to real elapsed time via a monotonic source (`Environment.TickCount64`/`Stopwatch`), `credited = min(wallDelta, monotonicElapsed + small slack)`. The current `ClockRollbackTests` can't see this (they advance the synthetic clock 10 s/tick — the honest cadence). | Critical → **High** (single-jump + tamper closed; within-ceiling creep OPEN until the monotonic bound lands + live test) |
