...
- 🟡 **Audit fixes #2/#3/#4/#10 + notifier early-lift fix (2026-06-15) — BENCH-VERIFIED (281/281 + 2 verifier ACCEPT), RE-SMOKE PENDING.**
  #2 timer re-entrancy (`Monitor.TryEnter` skip-guard), #3 atomic ini Save
  (temp + `File.Move` + bounded retry), #4 heartbeat TOCTOU re-validate, #10
  `DefineTrace=false` (Release Trace key/MAC leak off). The re-smoke surfaced a
  real EARLY-LIFT: the notifier clock-comp could write `[Time] Until` into the
  PAST (→ early lift) or, with a never-shorten clamp, OVER-RUN; **fixed by
  removing the comp — B4's HighWater owns clock-rollback, the notifier now only
  toggles `TimeChanging`.** Smoke test gained an early-lift guard (62 / 64-clock).
  **The on-time-lift re-smoke has NOT run — it is the next session's first task
  (see the 2026-06-15 header).** Dead notifier helpers (`ComputeCompensatedUntil`
  etc.) retained, delete with their `NotifierClockCompensationTests` later.
...
- **The notifier's clock-change compensation is OBSOLETE given B4 — REMOVED
  15/06/2026.** B4 decides expiry off the monotonic HighWater mark, not the wall
  clock, so rewriting `[Time] Until` on a clock change is redundant AND harmful:
  it caused a ~3 min EARLY LIFT (a backward clock jiggle, after the service wrote
  a jumped `[CurrentTime] Now`, made `ComputeCompensatedUntil` return a past
  `Until` below HighWater) and, with a never-shorten clamp, an OVER-RUN.
  `SystemEvents_TimeChanged` now only toggles the `TimeChanging` cooperation flag.
  Lesson: with B4 in place, do NOT let any user-session writer move `Until` off
  the wall clock — the monotonic mark already ends the block after the right real
  duration across any clock change.
- **The smoke test only checked THAT a block lifted, never WHEN — an early lift
  passed as 63/0.** Added an early-lift guard (`run-smoketest.ps1`: lift time must
  be ≥ blockStart + 285s for the `--for 5` block); a clock-drill early-lift
  regression now FAILS the smoke instead of hiding. Counts 61→62 / 63→64.
