...
Canonical source: the bypass table at vault/dev/monk-mode/specs/ARCHITECTURE.md:182-194. Never assess a bypass from memory — read the row first; every claim below cites it.

## Status board — DATED SNAPSHOT (06/07/2026)
This table is a convenience snapshot only; ARCHITECTURE.md:182-194 is the source of truth. Read the live row before acting on any entry here.
...
| B4 | Clock roll forward | Low | 63/63 incl. clock drill 14/06/2026; creep cap unit-pinned |
...
- [ ] Freshest dated evidence wins: repo CLAUDE.md:16 still says "63/63, 14/06/2026" — SUPERSEDED by the 71/0 whole-stack re-verification of 01/07/2026 (vault/dev/monk-mode/tasks.md:15). Cite 71/0.
- [ ] Distinguish live-verified (elevated smoke drill) from unit-pinned (test only) from assessed (read-only analysis, doc-only). B3's Safe-Mode reboot and B4's creep cap and the B1b backward-roll fix are NOT live-drilled — say so.
- [ ] Pending drills: B1c backward-clock smoke sits in the gated batch (plans/fable5-slices.md:39, :118) with E3/H3 (E5 is a gated external review, not a smoke — fable5-slices.md:80).

## Everything chains to B1
- [ ] If the service + guardian both stay dead, the B2 hosts, B3 SafeBoot, B5a DoH and B6 ACE self-heals all stop — each of those rows explicitly chains its residual to B1 (ARCHITECTURE.md:185, 186, 188, 189).
...
- [ ] B1's own residuals (table row :184): scripted near-simultaneous double-kill, `sc failure ... reset= 0`, suspend-then-kill, elevated guardian pre-pin. True kill-immunity = PPL/kernel driver = B10-tier, out of scope.

## Accepted — do not "improve"
- [ ] B10 offline/admin attack: honestly out of scope by design (README + table row :193).
- [ ] B11 fixed identifiers (`MONKMODE`, hosts marker, mutex, ini name, `mm_textbox`): accepted 02/07/2026 — obfuscation buys nothing because every scripted teardown hits the same fail-closed gate a manual one hits (B1/B2/B4/B6/B7). Do NOT propose randomising/salting identifiers (:194). No secret leaks via logs (`DefineTrace=false` in all four projects).
