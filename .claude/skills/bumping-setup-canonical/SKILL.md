...
description: "Setup canonical bump, s-schema bump, SetupSchemaVersion, monkmode_setup.ini, new [Setup] field, SetupCanonicalFromIni, WriteSetupConfig, setup default — executes a version bump on the CLI-only MAC'd setup ini (the live D1b s2→s3 pattern). Use when adding/removing a MAC-covered field on the SETUP file (account defaults like Partner, CoolOffSeconds, DefaultSites), when SetupSchemaVersion must change, or when editing SetupCanonicalFromIni or any Setup* reader."
---
...
- `monkmode_setup.ini` — SETUP canonical (s-series; the dirty-tree D1b work is s2→s3), written and read by the CLI ONLY. The service NEVER reads it. THIS skill.
...
- Version const: `Public Const SetupSchemaVersion As String = "s3"` — MonkMode/Blocker.vb:1107. Ladder in the comment at Blocker.vb:1103-1106: s1 = C6a (Done + Partner) · s2 = C6c (CoolOffSeconds) · s3 = D1b (DefaultSites).
...
4. [ ] Bump `SetupSchemaVersion` (s3 → s4) and extend the ladder comment at :1103-1106 with the new step + owning slice.
