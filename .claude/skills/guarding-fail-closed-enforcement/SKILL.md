...
- [ ] All five per-tick self-heals (B2 hosts repair, B1 guardian peer, B3 SafeBoot, B5a DoH-off, B6 deny-DELETE ACE) gate on the ONE `enforcementHeld` local, computed once in `timer_Elapsed` — re-locate by that name, not by line.
- [ ] Since v1.1 S3a `enforcementHeld` = `BlockHeld(v9 mirror…) OrElse AnyBlockHeld(slots…)`. The v9 disjunct is kept deliberately as a **widening** (S3b/S8 own removing the mirror): the fold can only ever ADD holds, never drop one.
- [ ] `BlockHeld` = `Not EffectiveBlockHasExpired(...) OrElse (macValid AndAlso ScheduleActive(...))` — for a manual block it reduces to the verbatim `Not EffectiveBlockHasExpired(...)` pattern; invalid MAC ⇒ held ⇒ keep enforcing.
- [ ] `AnyBlockHeld(slots, …)` answers `Not macValid` **before** the loop — with an empty/unreadable slot list a plain "does any slot hold?" fold returns False, which would stand every self-heal down on a frozen config. There are THREE predicates and conflating them is a proven fail-open: `SlotHeld`/`AnyBlockHeld` = keep the machinery up (PENDING counts) · `SlotEnforcesNow` = is this slot's own timer running (PENDING excluded) · **`SlotContributesLists` = `SlotEnforcesNow OrElse SlotIsPending`, and THAT is what the hosts/kill/url unions and the P37 snapshot-truth filter on.** The unions must include PENDING: until the S3b activation stamper exists, filtering them on `SlotEnforcesNow` strips a delayed slot's sites out of the snapshot and **nothing can ever put them back**, so every later hosts repair/reboot rebuilds the block without them (caught by the S3a verifier, 12/08/2026).
