'    MonkMode - ServiceSecurity (B6: service-deletion resistance) - SERVICE COPY
'
'    The PURE, unit-testable half of B6 - the SDDL string surgery that adds /
'    removes a single DELETE-deny ACE on the MONKMODE service object.
'
'    This file is byte-for-byte identical to MonkMode/ServiceSecurity.vb (the CLI
'    copy), like the Simple3Des / ConfigIntegrity copies across the projects -
'    only the RootNamespace differs (monkmode here, MonkMode in the CLI). The
'    unit tests pin that 2-copy parity (a drift would mean the service and the
'    escape hatch disagree on what "the deny ACE" is). The service has no project
'    reference to the CLI (the house rule: the exes never reference each other),
'    so the service-owned re-assert/re-grant uses THIS copy; the CLI escape hatch
'    uses the CLI copy.
'
