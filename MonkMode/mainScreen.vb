...
Public Class MonkMode
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If My.Computer.FileSystem.FileExists(Application.StartupPath + "\monkmode_settings.ini") = True Then
            Try
        ...
        Try
            ...
            If serviceController.Status = ServiceProcess.ServiceControllerStatus.Running Then
                ...
                answer = MsgBox("You are still blocked for: " + secondsToText(secondsStillBlocked, False) + vbNewLine + "Do you want to block more sites?", MsgBoxStyle.YesNo, "MonkMode")
    ...
    Private Sub goMonkMode(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Try
            If serviceController.Status = ServiceProcess.ServiceControllerStatus.Running Then
                MsgBox("Please wait until your current ban is lifted.", MsgBoxStyle.Information, "MonkMode")
                End
            End If
    ...
    Private Sub startBlock()
        ...
        If testWorked = True Then
            ...
            If chk_24.Checked = True Then
                MsgBox("You are now going MonkMode until " + hour24.Text + ":" + minute.Text + " on " + vbNewLine + DateTimePicker1.Text + "." + vbNewLine + vbNewLine _
                    + "You might not see the block until you close and reopen your all browser windows. You can see how much time you have left and add more sites by running MonkMode again.", MsgBoxStyle.Information, "MonkMode")
            Else
                MsgBox("You are now going MonkMode until " + hour.Text + ":" + minute.Text + " " + amPmSelector.Text + " on " + vbNewLine + DateTimePicker1.Text + "." + vbNewLine + vbNewLine _
                    + "You might not see the block until you close and reopen your all browser windows. You can see how much time you have left and add more sites by running MonkMode again.", MsgBoxStyle.Information, "MonkMode")
            End If
        Else
    ...
    Private Sub startService()
        ...
        If svcAlreadyExists = False Then
            ServiceInstaller.InstallAndStart("MONKMODE", "MONKMODE", Application.StartupPath + "\MonkMode_srv.exe")
        Else
            ServiceInstaller.StartService("MONKMODE")
        End If
        Try
            Dim serviceController As System.ServiceProcess.ServiceController
            Dim timeSpan As New TimeSpan(0, 0, 31)

            serviceController = New System.ServiceProcess.ServiceController("MONKMODE")
            ...
            Try
                ...
                runMe.SetValue("MonkMode_notify", Application.StartupPath + "\mm_notify.exe")
            Catch ex As Exception
                MsgBox("There was an error writing the mm_notify.exe startup entry")
            End Try

            Try
                Threading.Thread.Sleep(500) 'To offset the periodical reading times of monkmode_settings.ini between MONKMODE and mm_notify
                Process.Start(Application.StartupPath + "\mm_notify.exe")
                Process.Start(Application.StartupPath + "\mm_notify2.exe")
            Catch ex As Exception
                MsgBox("There was an error starting the notification program. MonkMode will still work, but you won't be notified when your block is over.")
            End Try
        ...
        Catch ex As Exception
            MsgBox("The service could not be started. This problem is likely caused by your antivirus software, but please don't disable your antivirus to run this program." + vbNewLine + vbNewLine + _
                   "You can try to add an antivirus exception to allow access to the hosts file by both 'MonkMode.exe' and 'monkmode.exe' in the MonkMode program folder, then try again.")
            erroredOut()
        End Try
    ...
    Private Sub lbl_info_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lbl_info.Click
        ...
        message = "MonkMode (0.7 Beta) Serious Edition" + vbNewLine + "(c) Copyright Felix Belzile, 2012" + vbNewLine + vbNewLine + "MonkMode will temporarily block addicting sites and games" + vbNewLine + "so that you can get some work done!"
        MsgBox(message, vbOKOnly, "About MonkMode")
    End Sub
    ...
    Private Sub MonkMode_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown

        Dim serverVersion As Double
        Dim thisVersion As Double = 0.7
        Dim answer As MsgBoxResult
        Try
            serverVersion = GetHTML("http://monkmode.local/version.html")
            If serverVersion > thisVersion Then
                answer = MsgBox("Looks like there's an update for MonkMode!" + vbNewLine + "Would you like to update now?", MsgBoxStyle.YesNo)
                If answer = MsgBoxResult.Yes Then
                    Process.Start("http://monkmode.local/download.html")
                    End
                End If
            End If
        Catch ex As Exception
